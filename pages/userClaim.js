...
export default function userClaim({cookies}) {
  ...
  return (
    <div  style={{width : '100%', height : '100vh', display : 'flex', justifyContent : 'space-between', alignItems : 'center', flexFlow : 'column nowrap'}}>
      ...
      <UserTopbar json = {{router :'claim', cookies : cookies }} />
      <div style={{width : '100%', display : 'flex', justifyContent : 'center', alignItems : 'center', fontSize : '2rem', padding : '3rem'}}>
        답변 대기중
      </div>
      <UserContentList cookies={cookies} check={false} router={'claim'} /> 
      <div style={{width : '100%', display : 'flex', justifyContent : 'center', alignItems : 'center', fontSize : '2rem', padding : '3rem'}}>
        답변완료
      </div>
      <UserContentList cookies={cookies} check={true} router={'claim'} /> 
      <Footer />
    </div>
  )
}
