...
export default function UpdateProduct({postId, cookies}) {
  ...
  useEffect(async() => {
    if(cookies.accessToken) {
      ...
      try {
        const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/product/${postId}`)
        setGetData(data)
        setTitle(data.product.title)
        setDatas(data.product.descriptions)
        setStr(data.product.descriptions)
  ...
  const onSubmit = () => {
    if(datas.length < 1 || title.length < 1) {
      ...
      console.log(str)
      axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/product/${userInfo.id}`,
      {
        postId : getData.product.id,
        title : title,
        descriptions : datas,
