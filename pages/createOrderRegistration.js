...
export default function CreateOrderRegistration({cookies}) {
  ...
  //라우터 값이 존재할 때
  useEffect(() => {
    if(router.asPath === '/createOrderRegistration') {
      setDisabled(false)
    }
  }, [router.asPath])
  ...
  const onSubmit = () => {
    if(data.length < 1 || title.length < 1) {
      ...
      axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/order_registration/${userId}`,
      {
        title : title,
        descriptions : data,
  ...
  return (
    <>
    <Topbar json = {{router :'OrderRegistration', cookies : cookies }} />
      <InputTitle 
      title={title}
      onChange={(data) => {
        setTitle(data)
