...
export default function DeleteProduct({cookies, postId}) {
  ...
  const onSubmit = () => {
    ...
    if(checkDelete) {
      axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/product/${userInfo.id}`,
      {
        data : {
          postId : postId
        },
  ...
  //홈으로 이동
  const onCancel = () => {
    router.push(`/productView/${postId}`)
  }
