...
export default function DeleteBusinessApproval({cookies, postId}) {
  ...
  const onSubmit = () => {
    ...
    if(checkDelete) {
      axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/business_approval/${userInfo.id ? userInfo.id : 0}/${postId}/${password ? password : 'password'}`,
      {
        data : {
          postId : postId
        },
  ...
  //홈으로 이동
  const onCancel = () => {
    router.push(`/businessApprovalView/${postId}`)
  }
