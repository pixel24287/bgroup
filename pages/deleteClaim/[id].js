...
export default function DeleteClaim({cookies, postId}) {
  ...
  const onSubmit = () => {
    let checkDelete = confirm('삭제하시겠습니까?')
      if(checkDelete) {
      axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/claim/${userInfo.id ? userInfo.id : 0}/${postId}/${password ? password : 'password'}`,
      {
        data : {
          postId : postId
        },
