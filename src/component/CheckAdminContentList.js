...
export const CheckAdminContentList = ({cookies, check, router}) => {
  ...
  useEffect(async () => {
    if (check) {
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${router}/check/${pageNumber - 1}`)
      setContents(data)
    } else {
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${router}/no/check`)
      setContents(data)
    }
  }, [pageNumber])
  ...
  const excelDownload = async (router) => {
    ...
    if (check) {
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${router}/check/list/${userInfo.id}`,{
        headers : {
      ...
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${router}/no/check/list/${userInfo.id}`,{
        headers : {
          'Content-type': 'application/json',
          'Accept': 'application/json',
