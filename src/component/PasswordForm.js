...
export const PasswordForm = ({cookies, userInfo ,postId, route , password ,setPassword , setIsLogin, setData}) => {
  ...
  useEffect(() => {
    if(userInfo.isAdmin) {
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${route}/${userInfo.id}/${postId}/password`,
      {
        headers: {
  ...
  const onSubmit = (e) => {
    ...
    if(password.length < 1) {
      ...
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${route}/${userInfo.id ? userInfo.id : 0}/${postId}/${password}`,
      {
        headers: {
          token: `Bearer ${cookies.accessToken}`
        }
