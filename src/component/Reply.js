...
export const Reply = ({data, cookies, route}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(description.length < 1) {
            ...
            //axios 로 비동기 처리 보내기
            axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/${route}_comment/reply/${userInfo.id}`,
            {
    ...
    const deleteSubmit = async () => {
        axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/${route}_comment/reply/${userInfo.id}`,
        {
            data : {
                comment_id : data.id
            },
