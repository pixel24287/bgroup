...
export const CheckAdminPagination = ({pageNumber, setPageNumber, router}) => {
    const [pages, setPages] = useState([])
    const [totalPageGroup, setTotalPageGroup] = useState(0)

    useEffect(async () => {
        const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${router}/list/total_page`)
