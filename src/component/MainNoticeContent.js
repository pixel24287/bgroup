...
export const MainNoticeContent = ({json}) => {
  ...
  const createSubmit = async () => {
    axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/notice/main/${userInfo.id}`,
    {
      id : json.val.id,
    },{
      headers:{
  ...
  const deleteSubmit = async () => {
    axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/notice/main/${userInfo.id}/${json.val.id}`,
    {
    },{
      headers:{
        'Content-type': 'application/json',
