...
export const PagePagination = ({pageNumber, setPageNumber, router, page_ids}) => {
    const [pages, setPages] = useState([])
    const [totalPageGroup, setTotalPageGroup] = useState(0)
    const [page_id, setPage_id] = useState(page_ids)
    useEffect(async () => {
        const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${router}/total_page/${page_id}`)
